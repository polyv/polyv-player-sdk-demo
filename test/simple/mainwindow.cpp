...
MainWindow::MainWindow(QWidget *parent)
	: QMainWindow(parent), ui(new Ui::MainWindow), settings(GetConfigPath(), QSettings::IniFormat)
{
	...
	connect(ui->subAccountCheckBox, &QCheckBox::stateChanged, this, [this]() {
...
void MainWindow::OnShowMsg(const QString &msg, int level)
{
	...
	if (ui->autoScrollCheckBox->isChecked()) {
		ui->msgPlainTextEdit->verticalScrollBar()->setValue(
			ui->msgPlainTextEdit->verticalScrollBar()->maximum());
	}
}
...
void MainWindow::on_initPushButton_clicked()
{
	...
	PLVSetSdkLogMessageCallback(
		...
		[](LOG_FILTER_TYPE level, const char *message, void *data) {
			...
			QMetaObject::invokeMethod(main, "OnShowMsg", Q_ARG(const QString &, message),
						  Q_ARG(int, level));
		},
		this);
...
void MainWindow::on_detectSoftwareRecordCheckBox_clicked()
{
	...
	PLVSetDetectSoftwareRecordingHandler(
		...
		[](SOFTWARE_RECORDING_NOTIFY_TYPE type, const char *softwares, void *data) {
			...
			QString msg =
				QString("software recording, type:%1, softwares:%2").arg(type).arg(QT_UTF8(softwares));
			QMetaObject::invokeMethod(obj, "OnShowMsg", Q_ARG(const QString &, msg),
						  Q_ARG(int, MsgCallback));
		},
		this);
...
void MainWindow::OnVideoInfo(int code, QVariantList videoList, QVariantMap pageInfo)
{
	...
	ui->pageLabel->setText(
		QString("%1/%2").arg(pageInfo.value("pageNumber").toInt()).arg(pageInfo.value("totalPages").toInt()));
}
...
void MainWindow::on_tokenRequestPushButton_clicked()
{
	...
	getTokenThread = std::thread([this, vid]() {
		QString resultStr;
...
void MainWindow::on_seekToEndPushButton_clicked()
{
	if (IsPlaying()) {
...
void MainWindow::on_shotScreenPushButton_clicked()
{
	if (IsPlaying()) {
		QString filepath =
			GetScreenshotPath() + "/" + QDateTime::currentDateTime().toString("yyyy-MM-dd-hh-mm-ss.jpg");
...
void MainWindow::on_subtitleComboBox_currentIndexChanged(int index)
{
	PLVPlayerSetCurrentSubtitle(player, QT_TO_UTF8(ui->subtitleComboBox->itemData(index).toString()));
}

void MainWindow::on_subtitlePushButton_clicked()
{
	auto textFont = settings.value("subtitleTextFont", "sans-serif").toString();
	auto fontDB = std::make_shared<QFontDatabase>();
	auto familys = fontDB->families();
	if (!familys.contains(textFont)) {
		familys.insert(0, textFont);
	}
	auto textFontLabel = new QLabel(QTStr("Font"));
	auto textFontCombox = new QComboBox();
	textFontCombox->setMaxVisibleItems(20);
	textFontCombox->addItems(familys);
	textFontCombox->setCurrentIndex(textFontCombox->findText(textFont));
	auto textSize = settings.value("subtitleTextSize", 38).toInt();
	auto textSizeLabel = new QLabel(QTStr("TextSize"));
	auto textSizeEdit = new QLineEdit(QString::number(textSize));
	auto textColor = settings.value("subtitleTextColor", "#FFFFFFFF").toString();
	auto textColorLabel = new QLabel(QTStr("TextColor"));
	auto textColorEdit = new QLineEdit(textColor);
	auto borderSize = settings.value("subtitleBorderSize", 2).toInt();
	auto borderSizeLabel = new QLabel(QTStr("BorderSize"));
	auto borderSizeEdit = new QLineEdit(QString::number(borderSize));
	auto borderColor = settings.value("subtitleBorderColor", "#FF000000").toString();
	auto borderColorLabel = new QLabel(QTStr("BorderColor"));
	auto borderColorEdit = new QLineEdit(borderColor);
	auto backgroundColor = settings.value("subtitleBackgroundColor", "#00000000").toString();
	auto backgroundColorLabel = new QLabel(QTStr("BackgroundColor"));
	auto backgroundColorEdit = new QLineEdit(backgroundColor);
	auto alignX = settings.value("subtitleAlignX", 0).toInt();
	auto alignXLabel = new QLabel(QTStr("HPos"));
	auto alignXCombox = new QComboBox();
	QList<QPair<QString, int>> alignXs{{QTStr("Left"), -1}, {QTStr("Middle"), 0}, {QTStr("Right"), 1}};
	for (const auto &p : alignXs) {
		alignXCombox->addItem(p.first, p.second);
	}
	alignXCombox->setCurrentIndex(alignXCombox->findData(alignX));
	auto alignY = settings.value("subtitleAlignY", 1).toInt();
	auto alignYLabel = new QLabel(QTStr("VPos"));
	auto alignYCombox = new QComboBox();
	QList<QPair<QString, int>> alignYs{{QTStr("Top"), -1}, {QTStr("Middle"), 0}, {QTStr("Bottom"), 1}};
	for (const auto &p : alignYs) {
		alignYCombox->addItem(p.first, p.second);
	}
	alignYCombox->setCurrentIndex(alignYCombox->findData(alignY));
	auto posY = settings.value("subtitlePosY", 100).toInt();
	auto posYLabel = new QLabel(QTStr("VAdjust"));
	auto posYEdit = new QLineEdit(QString::number(posY));
	auto scaleByWindow = settings.value("subtitleScaleByWindow", true).toBool();
	auto scaleByWindowLabel = new QLabel(QTStr("ScaleByWindow"));
	auto scaleByWindowCheck = new QCheckBox();
	scaleByWindowCheck->setChecked(scaleByWindow);
	auto visible = settings.value("subtitleVisible", true).toBool();
	auto visibleLabel = new QLabel(QTStr("SubtitleVisible"));
	auto visibleCheck = new QCheckBox();
	visibleCheck->setChecked(visible);
	auto contentWidget = new QWidget();
	auto contentLayout = new QFormLayout(contentWidget);
	contentLayout->setContentsMargins(0, 0, 0, 0);
	contentLayout->setHorizontalSpacing(6);
	contentLayout->setVerticalSpacing(8);
	contentLayout->addRow(textFontLabel, textFontCombox);
	contentLayout->addRow(textSizeLabel, textSizeEdit);
	contentLayout->addRow(textColorLabel, textColorEdit);
	contentLayout->addRow(borderSizeLabel, borderSizeEdit);
	contentLayout->addRow(borderColorLabel, borderColorEdit);
	contentLayout->addRow(backgroundColorLabel, backgroundColorEdit);
	contentLayout->addRow(alignXLabel, alignXCombox);
	contentLayout->addRow(alignYLabel, alignYCombox);
	contentLayout->addRow(posYLabel, posYEdit);
	contentLayout->addRow(scaleByWindowLabel, scaleByWindowCheck);
	contentLayout->addRow(visibleLabel, visibleCheck);

	// dialog
	QDialog dialog(this);
	dialog.setWindowFlags(dialog.windowFlags() & ~Qt::WindowContextHelpButtonHint);
	dialog.setWindowTitle(QTStr("SubtitleConfig"));
	...
	// config
	textFont = textFontCombox->currentText();
	textSize = textSizeEdit->text().toInt();
	textColor = textColorEdit->text();
	borderSize = borderSizeEdit->text().toInt();
	borderColor = borderColorEdit->text();
	backgroundColor = backgroundColorEdit->text();
	alignX = alignXCombox->currentData().toInt();
	alignY = alignYCombox->currentData().toInt();
	posY = posYEdit->text().toInt();
	scaleByWindow = scaleByWindowCheck->isChecked();
	visible = visibleCheck->isChecked();
	PLVSubtitleConfigInfo config;
	std::string textFontStd = textFont.toStdString();
	config.textFont = textFontStd.c_str();
	config.textSize = textSize;
	std::string textColorStd = textColor.toStdString();
	config.textColor = textColorStd.c_str();
	config.borderSize = borderSize;
	std::string borderColorStd = borderColor.toStdString();
	config.borderColor = borderColorStd.c_str();
	std::string backgroundColorStd = backgroundColor.toStdString();
	config.backgroundColor = backgroundColorStd.c_str();
	config.alignX = alignX;
	config.alignY = alignY;
	config.posY = posY;
	config.scaleByWindow = scaleByWindow;
	config.visible = visible;
	PLVPlayerSetSubtitleConfig(player, &config);
	settings.setValue("subtitleTextFont", textFont);
	settings.setValue("subtitleTextSize", textSize);
	settings.setValue("subtitleTextColor", textColor);
	settings.setValue("subtitleBorderSize", borderSize);
	settings.setValue("subtitleBorderColor", borderColor);
	settings.setValue("subtitleBackgroundColor", backgroundColor);
	settings.setValue("subtitleAlignX", alignX);
	settings.setValue("subtitleAlignY", alignY);
	settings.setValue("subtitlePosY", posY);
	settings.setValue("subtitleScaleByWindow", scaleByWindow);
	settings.setValue("subtitleVisible", visible);
	settings.sync();
}
...
void MainWindow::OnMediaState(QString vid, int state)
{
	...
	// set subtitles
	bool isLoaded = PLVPlayerIsLoaded(player);
	int subtitleCount = PLVPlayerGetSubtitleCount(player);
	ui->subtitleComboBox->setEnabled(isLoaded && subtitleCount > 0);
	if (state == MEDIA_STATE_LOADED) {
		ui->subtitleComboBox->blockSignals(true);
		ui->subtitleComboBox->clear();
		if (subtitleCount > 0) {
			for (int i = 0; i < subtitleCount; i++) {
				char name[PLV_MAX_NAME_LENGTH]{};
				if (PLVPlayerGetSubtitleName(player, i, name) == E_NO_ERR) {
					ui->subtitleComboBox->addItem(QT_UTF8(name), QT_UTF8(name));
				}
			}
			// current
			char curSubtitle[PLV_MAX_NAME_LENGTH]{};
			PLVPlayerGetCurrentSubtitle(player, curSubtitle);
			for (int i = 0; i < ui->subtitleComboBox->count(); i++) {
				if (ui->subtitleComboBox->itemData(i).toString() == QT_UTF8(curSubtitle)) {
					ui->subtitleComboBox->setCurrentIndex(i);
					break;
				}
			}
		}
		ui->subtitleComboBox->blockSignals(false);
	}
}
...
void MainWindow::OnMediaProperty(QString vid, int property, int format, QString value)
{
	...
	case MEDIA_PROPERTY_DURATION:
		duration = value;
		break;
	case MEDIA_PROPERTY_HWDEC:
		hwdec = value;
		break;
	case MEDIA_PROPERTY_VIDEO_CODEC:
		vCodec = value;
		break;
	case MEDIA_PROPERTY_VIDEO_BITRATE:
		vBitrate = value;
		break;
	case MEDIA_PROPERTY_VIDEO_FPS:
		fps = value;
		break;
	...
	case MEDIA_PROPERTY_VIDEO_HEIGHT:
		height = value;
		break;
	case MEDIA_PROPERTY_AUDIO_CODEC:
		aCodec = value;
		break;
	case MEDIA_PROPERTY_AUDIO_BITRATE:
		aBitrate = value;
		break;
	case MEDIA_PROPERTY_CACHE_SPEED:
		cacheSpeed = value;
		break;
	case MEDIA_PROPERTY_SUBTITLE_TEXT:
		subtitleContent = value;
		break;
	case MEDIA_PROPERTY_SUBTITLE_START:
		subtitleStart = value;
		break;
	case MEDIA_PROPERTY_SUBTITLE_STOP:
		subtitleStop = value;
		break;
	default:
		break;
	...
	QString playInfo = QString("Video:%1%2,%3x%4@%5-%6kbps,Audio:%7-%8kbps,Cache:%9KB/s")
				   ...
				   .arg(width)
				   .arg(height)
				   .arg(QString::number(fps.toFloat(), 'f', 2))
				   .arg(vBitrate.toLongLong() / 1024)
	...
	// subtitle
	auto formatMillsecondsToString = [](const QString &ms) {
		return QTime(0, 0).addMSecs(ms.toInt()).toString("hh:mm:ss.zzz");
	};
	QString subtitleMsg;
	if (!subtitleContent.isEmpty()) {
		subtitleMsg = QString("%1-%2:%3")
				      .arg(formatMillsecondsToString(subtitleStart))
				      .arg(formatMillsecondsToString(subtitleStop))
				      .arg(subtitleContent);
	}
	ui->subtitleLabel->setText(subtitleMsg);
	ui->subtitleLabel->setToolTip(subtitleMsg);
}
...
void MainWindow::OnMediaRateChanged(QString vid, int inputRate_, int realRate)
{
	...
	OnShowMsg(QString("vid:%1,input rate:%2, real rate:%3")
			  .arg(vid)
			  .arg(GetRateName(inputRate))
			  .arg(GetRateName(currentRate)),
		  MsgCallback);
}
...
void MainWindow::OnMediaProcess(QString vid, int ms)
{
	...
	QString playTime = QString("%1/%2")
				   .arg(GetTimeString(position.toLongLong() / 1000))
				   .arg(GetTimeString(duration.toLongLong() / 1000));
...
{
	QString path =
		QFileDialog::getExistingDirectory(this, QTStr("ChooseVideoPath"), ui->localVideoPathLineEdit->text());
	if (!path.isEmpty()) {
...
void MainWindow::OnAddDownloader(QVariantMap video, QVariantMap rateInfo, qint64 speedLimit)
{
	...
	for (int i = 0; i < row; i++) {
		auto detailsWidget =
			qobject_cast<SimpleDownloadDetails *>(ui->downloadVideoTableWidget->cellWidget(i, 0));
	...
	PLVDownloadSetErrorHandler(
		...
		[](const char *vid, int code, void *data) {
			QString msg = QString("download error! ptr:%1, vid:%2, code:%3")
					      ...
					      .arg(QT_UTF8(vid))
					      .arg(code);
			QMetaObject::invokeMethod(MainWindow::main, "OnShowMsg", Q_ARG(const QString &, msg),
						  Q_ARG(int, MsgCallback));
		},
		downloader);
	PLVDownloadSetProgressHandler(
		...
		[](const char *vid, long long receivedBytes, long long totalBytes, void *data) {
			QMetaObject::invokeMethod(MainWindow::main, "OnDownloaderProgress", Q_ARG(qint64, (qint64)data),
						  Q_ARG(QString, QT_UTF8(vid)), Q_ARG(qint64, receivedBytes),
	...
	PLVDownloadSetResultHandler(
		...
		[](const char *vid, int rate, int code, void *data) {
			QMetaObject::invokeMethod(MainWindow::main, "OnDownloaderComplete", Q_ARG(qint64, (qint64)data),
						  Q_ARG(QString, QT_UTF8(vid)), Q_ARG(int, rate), Q_ARG(int, code));
		},
		downloader);
	...
	connect(operatorWidget, &SimpleDownloadOperator::start, this, [this]() {
		auto downloader = sender()->property("downloader").toLongLong();
		//token
		int index = FindDownloader(downloader);
		auto detailsWidget =
			qobject_cast<SimpleDownloadDetails *>(ui->downloadVideoTableWidget->cellWidget(index, 0));
	...
	connect(operatorWidget, &SimpleDownloadOperator::stop, this, [this]() {
		auto downloader = (PLVDownloadPtr)(sender()->property("downloader").toLongLong());
		PLVDownloadStop(downloader);
	});
	connect(operatorWidget, &SimpleDownloadOperator::pause, this, [this]() {
		auto downloader = (PLVDownloadPtr)(sender()->property("downloader").toLongLong());
		PLVDownloadPause(downloader, true);
	});
	connect(operatorWidget, &SimpleDownloadOperator::resume, this, [this]() {
		auto downloader = (PLVDownloadPtr)(sender()->property("downloader").toLongLong());
		PLVDownloadPause(downloader, false);
	});
	connect(operatorWidget, &SimpleDownloadOperator::openDir, this, [this]() {
		auto downloader = (sender()->property("downloader").toLongLong());
		int index = FindDownloader(downloader);
		auto detailsWidget =
			qobject_cast<SimpleDownloadDetails *>(ui->downloadVideoTableWidget->cellWidget(index, 0));
	...
	connect(operatorWidget, &SimpleDownloadOperator::play, this, [this]() {
		auto downloader = (sender()->property("downloader").toLongLong());
		int index = FindDownloader(downloader);
		auto detailsWidget =
			qobject_cast<SimpleDownloadDetails *>(ui->downloadVideoTableWidget->cellWidget(index, 0));
	...
	connect(operatorWidget, &SimpleDownloadOperator::deleted, this, [this]() {
		auto downloader = (PLVDownloadPtr)(sender()->property("downloader").toLongLong());
...
void MainWindow::OnDownloaderProgress(qint64 downloader, QString vid, qint64 current, qint64 total)
{
	...
	for (int i = 0; i < row; i++) {
		auto processWidget =
			qobject_cast<SimpleDownloadProcess *>(ui->downloadVideoTableWidget->cellWidget(i, 1));
...
void MainWindow::OnCustomRenderUpdate()
{
	if (renderBufferFill.load()) {
		ui->playOpenGLWidget->updateImage(QImage(renderFrame.data[0], renderFrame.width, renderFrame.height,
							 renderFrame.linesize[0], QImage::Format_RGBX8888));
		renderBufferFill.exchange(false);
	}
}
...
bool MainWindow::eventFilter(QObject *obj, QEvent *event)
{
	if (obj == ui->videoTableWidget && event->type() == QEvent::ContextMenu) {
		...
		if (ui->videoTableWidget->isEnabled() && item) {
			...
			connect(playCustomAction, &QAction::triggered, this, [this, video]() {
				QString vid = video.value("vid").toString();
				bool encrypt = video.value("encrypt").toBool();
			...
			for (auto &item : video.value("rates").toList()) {
				...
				// normal speed
				auto action = downloadRatesMenu->addAction(rateName +
									   QString(" %1").arg(QTStr("NoSpeedLimit")));
				connect(action, &QAction::triggered, this,
					[this, downVideo, rateInfo]() { OnAddDownloader(downVideo, rateInfo, 0); });
				// limit speed
				action = downloadRatesMenu->addAction(rateName +
								      QString(" %1").arg(QTStr("SpeedLimit")));
				connect(action, &QAction::triggered, this,
...
void MainWindow::RequestVideoList(PLVVideoRequestParam *param)
{
	PLVRequestVideoInfo(
		true, param,
		[](int code, const PLVVideoRequestInfo infos[], int infosNum, const PLVVideoRequestPageInfo *pageInfo,
		   void *data) {
			auto main = static_cast<MainWindow *>(data);
			QMetaObject::invokeMethod(
				main, "OnVideoInfo", Q_ARG(int, code),
				Q_ARG(QVariantList,
				      code == E_NO_ERR ? MakeVideoInfoList(infos, infosNum) : QVariantList()),
...
{
	QMap<QString, QString> params{{"vid", vid},
...
void MainWindow::InitPlayer()
{
	...
	PLVPlayerSetPropertyHandler(
		...
		[](const char *vid, int property, int format, const char *value, void *data) {
			auto obj = static_cast<MainWindow *>(data);
			QMetaObject::invokeMethod(obj, "OnMediaProperty", Q_ARG(QString, QT_UTF8(vid)),
						  Q_ARG(int, property), Q_ARG(int, format),
	...
	PLVPlayerSetRateChangeHandler(
		...
		[](const char *vid, int inputRate, int realRate, void *data) {
			auto obj = static_cast<MainWindow *>(data);
			QMetaObject::invokeMethod(obj, "OnMediaRateChanged", Q_ARG(QString, QT_UTF8(vid)),
						  Q_ARG(int, inputRate), Q_ARG(int, realRate));
		},
		this);
	PLVPlayerSetProgressHandler(
		...
		[](const char *vid, int millisecond, void *data) {
			auto obj = static_cast<MainWindow *>(data);
			QMetaObject::invokeMethod(obj, "OnMediaProcess", Q_ARG(QString, QT_UTF8(vid)),
	...
	// subtitle config
	PLVSubtitleConfigInfo sc{};
	auto textFont = settings.value("subtitleTextFont", "sans-serif").toString().toStdString();
	sc.textFont = textFont.c_str();
	sc.textSize = settings.value("subtitleTextSize", 38).toInt();
	auto textColor = settings.value("subtitleTextColor", "#FFFFFFFF").toString().toStdString();
	sc.textColor = textColor.c_str();
	sc.borderSize = settings.value("subtitleBorderSize", 2).toInt();
	auto borderColor = settings.value("subtitleBorderColor", "#FF000000").toString().toStdString();
	sc.borderColor = borderColor.c_str();
	auto backgroundColor = settings.value("subtitleBackgroundColor", "#00000000").toString().toStdString();
	sc.backgroundColor = backgroundColor.c_str();
	sc.alignX = settings.value("subtitleAlignX", 0).toInt();
	sc.alignY = settings.value("subtitleAlignY", 1).toInt();
	sc.posY = settings.value("subtitlePosY", 100).toInt();
	sc.scaleByWindow = settings.value("subtitleScaleByWindow", true).toBool();
	sc.visible = settings.value("subtitleVisible", true).toBool();
	PLVPlayerSetSubtitleConfig(player, &sc);
}
...
void MainWindow::on_livePlayPushButton_clicked()
{
	...
	PLVLivePlayerSetToken(livePlayer,
			      QT_TO_UTF8(ui->livePlayTokenLineEdit->text())); // you can update token during playing.
...
void MainWindow::OnLiveChannelInfo(QVariantMap info)
{
	...
	for (int i = 0; i < lines.size(); i++) {
		ui->liveLineComboBox->addItem(QString("%1 %2").arg(QTStr("ChannelLine")).arg(QString::number(i + 1)),
					      lines.at(i));
	}
	if (lines.empty()) {
		ui->liveLineComboBox->setEnabled(false);
...
{
	return (liveMediaState != MEDIA_STATE_NONE && liveMediaState != MEDIA_STATE_FAIL &&
		liveMediaState != MEDIA_STATE_END);
}
