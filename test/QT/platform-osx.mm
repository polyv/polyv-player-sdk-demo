...
string GetBundlePath()
{
	...
	return bundleURL.path.fileSystemRepresentation;

	// string output = [[bundleURL path] UTF8String];
	// if (!access(output.c_str(), R_OK))
	// 	return output;
	// else
	// 	return string();
}
...
void SetCollectionBehavior(QWidget *window, ns::NSWindowCollectionBehavior behavior, bool on)
{
	NSView *nsv = (__bridge NSView *)reinterpret_cast<void *>(window->winId());
	NSWindow *nsw = nsv.window;
	NSUInteger behaviors_flag = nsw.collectionBehavior;
	if (on)
		behaviors_flag |= behavior;
    else
        behaviors_flag &= ~behavior;
	[nsw setCollectionBehavior:behaviors_flag];
}
